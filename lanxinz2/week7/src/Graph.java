...
public class Graph implements AStarInterface<JsonCity> {
        ...
        public JsonCity getStart() {
            //get the start city from user
            TextIO.putln("Please choose a city in Illinois to start: ");
            String startCity = TextIO.getln();
            while(!nameToNodeIndexMap.containsKey(startCity)){
                TextIO.putln("Can't find " + startCity + " in Illinois");
                TextIO.putln("Please enter again: ");
                startCity = TextIO.getln();
            }
            int start = nameToNodeIndexMap.get(startCity);
            return nodes[start];
        }

        @Override
        public JsonCity getEnd() {
            //get the end city from user
            TextIO.putln("Please choose a city in Illinois to be the end: ");
            String endCity = TextIO.getln();
            while(!nameToNodeIndexMap.containsKey(endCity)){
                TextIO.putln("Can't find " + endCity + " in Illinois");
                TextIO.putln("Please enter again: ");
                endCity = TextIO.getln();
            }
            int end = nameToNodeIndexMap.get(endCity);
            return nodes[end];
        }
